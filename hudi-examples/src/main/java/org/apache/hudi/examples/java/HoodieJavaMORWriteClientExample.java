 ...
 */
public class HoodieJavaMORWriteClientExample {

  private static final Logger LOG = LogManager.getLogger(HoodieJavaMORWriteClientExample.class);
...
//    client.insert(writeRecords, newCommitTime);
    client.insertPreppedRecords(writeRecords, newCommitTime);

    // updates
    newCommitTime = client.startCommit();
    LOG.info("Starting commit " + newCommitTime);
    List<HoodieRecord<HoodieAvroPayload>> toBeUpdated = dataGen.generateUpdates(newCommitTime, 5);
    records.addAll(toBeUpdated);
    recordsSoFar.addAll(toBeUpdated);
    writeRecords =
        recordsSoFar.stream().map(r -> new HoodieRecord<HoodieAvroPayload>(r)).collect(Collectors.toList());
//    client.upsert(writeRecords, newCommitTime);
    client.upsertPreppedRecords(writeRecords, newCommitTime);
